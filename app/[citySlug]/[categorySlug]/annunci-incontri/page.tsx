...
const SUB_CATEGORY_DISPLAY_NAME = "Incontri";
...
interface IncontriListingsPageProps {
...
export async function generateMetadata({ params: paramsPromise }: IncontriListingsPageProps): Promise<Metadata> {
  ...
  const title = `Annunci ${SUB_CATEGORY_DISPLAY_NAME} ${categoryName} a ${cityName} - Trova Subito!`;
  const description = `Scopri annunci per ${SUB_CATEGORY_DISPLAY_NAME} ${categoryName} a ${cityName}. Connessioni autentiche e profili verificati.`;
  
  return {
    title,
    description,
    keywords: [`annunci ${SUB_CATEGORY_DISPLAY_NAME} ${categoryName} ${cityName}`, `incontri ${categoryName} ${cityName}`],
    openGraph: {
      title,
      description,
...
export default async function AnnunciIncontriPage({ params: paramsPromise }: IncontriListingsPageProps) {
  const { citySlug, categorySlug } = await paramsPromise; // Await the params
