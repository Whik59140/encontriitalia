...
const SUB_CATEGORY_SLUG = "annunci-incontri";
...
interface AnnunciIncontriPageProps {
...
export async function generateMetadata({ params: paramsPromise }: AnnunciIncontriPageProps): Promise<Metadata> {
  ...
  const title = annunciIncontriPageStrings.generateMetadataTitle(annunciIncontriPageStrings.subCategoryDisplayName, categoryName, cityName);
  const description = annunciIncontriPageStrings.generateMetadataDescription(annunciIncontriPageStrings.subCategoryDisplayName, categoryName, cityName);
  
  return {
    title,
    description,
    keywords: annunciIncontriPageStrings.generateMetadataKeywords(annunciIncontriPageStrings.subCategoryDisplayName, categoryName, cityName),
    openGraph: {
      title,
      description,
...
export default async function AnnunciIncontriPage({ params: paramsPromise }: AnnunciIncontriPageProps) {
  const { citySlug, categorySlug } = await paramsPromise; // Await the params
      ...
      <header className="py-4 bg-white dark:bg-gray-900 shadow-sm">
        <div className="container mx-auto px-4 flex items-center justify-between">
          <Link href={`/${citySlug}/${categorySlug}/annunci`} className="flex items-center text-pink-600 dark:text-pink-400 hover:underline">
            ...
            {annunciIncontriPageStrings.backLinkText(categoryDisplayName, cityDisplayName)}
          </Link>
        </div>
      </header>
